...
public class BanguiMenu {
    ...
    public static void openBanguiMainMenu(Player player, OfflinePlayer target){
        ...
        if(target.isOnline()){
            ...
            playerHead = new ItemBuilder(Material.PLAYER_HEAD, 1, (short)3).setSkullOwner(target.getName()).setName("§b" + target.getName()).setLore("§aEn ligne", "§fVie : " + onlineTarget.getHealth(), "§fNourriture : " + onlineTarget.getFoodLevel(), "§fIP : " + onlineTarget.getAddress().toString().split(":")[0].substring(1), "§fBanni : " + (target.isBanned() ? "Oui" : "Non"), "§fMute : " + (SanctionsManager.isMuted(target) ? "Oui" : "Non")).toItemStack();
        } else {
            playerHead = new ItemBuilder(Material.PLAYER_HEAD, 1, (short)3).setSkullOwner(target.getName()).setName("§b" + target.getName()).setLore("§7Hors ligne", "§fBanni : " + (target.isBanned() ? "Oui" : "Non") + (SanctionsManager.isMuted(target) ? "Oui" : "Non")).toItemStack();
        }
